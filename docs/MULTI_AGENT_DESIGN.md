...
Phase 4 ✅：加 Memory Agent（长会话摘要，降低 token 成本）
        ↓
Phase 5（改为轻量版）：原计划做成 LLM Strategy Selector，评估后认为会给
        本来确定性的策略选择引入不确定性，反而不利于可预测性。改为
        **轻量决策审计日志**：保持确定性选择，但记录每次选择的依据。
```
...
### Phase 5 已实现（轻量决策审计日志）

- **为什么不做 LLM Strategy Selector**：PM+ 策略选择本来是确定性规则（K6 维度 → 策略映射），
  用 LLM 重新决定会引入不确定性，对心理健康产品是退步。可审计的目标不需要 LLM。
- **做了什么**：`k6_scorer.select_next_pm_strategy_with_reason()` 在确定性选择之外
  返回「选择依据」；`Coordinator._log_decision()` 把每次决策（K6 完成选首策略、
  PM_DECISION 续选策略）写入 `session["decision_log"]`：
  ```json
  {"turn": 6, "event": "k6_completed", "selected": "pm_behavioral_activation",
   "reason": "K6「抑鬱」=3 為最高且未處理 → pm_behavioral_activation",
   "k6_scores": {...}, "k6_total": 10, "k6_severity": "moderate",
   "timestamp": "2026-..."}
  ```
- 用 `python tools/k6_query.py <手机号>` 可查看完整决策轨迹。
